<article class="flex border-b pb-4 justify-between">
    ...
    <div class="flex items-center justify-center flex-col-reverse">
        <div class="flex gap-2">
            @php($modalName = 'confirm-project-' . $project->id . '-deletion')
            <flux:button href="#" class="hover:cursor-pointer">View</flux:button>
            <flux:button href="{{ route('management.portfolio.edit', $project->slug) }}" class="hover:cursor-pointer">Edit</flux:button>
            ...
            <flux:modal name="{{ $modalName }}" :show="$errors->isNotEmpty()" focusable class="max-w-lg">
                <form wire:submit="delete" class="space-y-6">
                    <flux:heading size="lg">{{ __('Are you sure you want to delete project \'' . $project->title . '\'?') }}</flux:heading>
